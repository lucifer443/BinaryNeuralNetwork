...
class StrongBaselineBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, **kwargs):
        ...
        self.conv1 = StrongBaselineConv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False, **kwargs)
        self.scale1 = LearnableScale(out_channels)
        self.nonlinear1 = nn.PReLU(out_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.conv2 = StrongBaselineConv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, **kwargs)
...
class StrongBaselineFPBlock(nn.Module):
    """base block from real-to-binary net"""
    expansion = 1

    def __init__(self, in_channels, out_channels, stride=1, downsample=None, **kwargs):
        super(StrongBaselineFPBlock, self).__init__()
        self.bn1 = nn.BatchNorm2d(in_channels)

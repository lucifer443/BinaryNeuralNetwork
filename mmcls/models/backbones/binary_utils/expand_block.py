...
class EpBlockB(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, **kwargs):
        ...
        if planes != inplanes:
            self.expand2 = BiasExpand(planes // inplanes)
            self.conv1 = RAConv2d(inplanes * planes // inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False,
                                  **kwargs)
        else:
            self.conv1 = RAConv2d(inplanes * self.ratio, planes, kernel_size=3, stride=stride, padding=1, bias=False,
                                  **kwargs)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = RAConv2d(planes * self.ratio, planes, kernel_size=1, stride=1, padding=0, bias=False,
                              **kwargs)
        self.bn2 = nn.BatchNorm2d(planes)
        self.nonlinear = nn.Hardtanh(inplace=True)
    ...
    def forward(self, x):
        ...
        if self.inplanes == self.planes:
            ...
            identity = torch.cat([identity] * (self.planes // self.inplanes), dim=1)
            out1 = self.expand2(x)

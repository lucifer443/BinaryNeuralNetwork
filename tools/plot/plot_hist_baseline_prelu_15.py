...
def main():
    ...
    config = 'configs/zzp/baseline/baseline_prelu/baseline_prelu_15_b32x8/baseline_prelu_15_b32x8_step1.py'
    ckpt = 'work_dir/baseline/baseline_prelu/baseline_prelu_15_b32x8/baseline_prelu_15_b32x8_step1/epoch_75.pth'
    ...
    assert len(bn1_out) == 16
    assert len(prelu_out) == 16
    assert len(sum_out) == 16
    assert len(bn2_out) == 16

    # # show the results
    fig, axs = plt.subplots(6, 16, figsize=(64, 18))
    axs = axs.flat
    for i in range(16):
        ...
        axs[i+16].grid()
        axs[i+32].set_title(f'bn1_out_{i}')
        axs[i+32].hist(bn1_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+32].grid()
        ...
        axs[i+64].grid()
        axs[i+80].set_title(f'bn2_out_{i}')
        axs[i+80].hist(bn2_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+80].grid()
    fig.savefig('./work_dir/plot/feature_hist/feature_hist_baseline_prelu_15.jpg')

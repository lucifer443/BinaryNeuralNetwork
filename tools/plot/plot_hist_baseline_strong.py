...
from mmcls.models.backbones.binary_utils.binary_convs import RAConv2d, IRConv2d, BLConv2d, STEConv2d
...
def get_steconv_inout(module, fea_in, fea_out):
    global steconv_in
    global steconv_out
    # print(type(fea_in), type(fea_out))
    # fea_in is a tuple, fea_out is a tensor
    steconv_in.append(fea_in[0].cpu())
    steconv_out.append(fea_out.cpu())
...
def main():
    ...
    config = 'configs/zzp/baseline/baseline_strong/baseline_strong_hue01_b32x8/baseline_strong_hue01_b32x8_step1.py'
    ckpt = 'work_dir/baseline/baseline_strong/baseline_strong_hue01_b32x8/baseline_strong_hue01_b32x8_step1/epoch_75.pth'
    ...
    for m in model.modules():
        ...
        if isinstance(m, STEConv2d):
            m.register_forward_hook(hook=get_steconv_inout)
    ...
    assert len(bn_in) == 16
    assert len(steconv_in) == 16
    assert len(steconv_out) == 16
    ...
    for i in range(16):
        ...
        axs[i+16].hist(steconv_in[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+16].grid()
        axs[i+32].set_title(f'conv_out_{i}')
        axs[i+32].hist(steconv_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+32].grid()
        ...
        axs[i+64].grid()
    fig.savefig('./work_dir/plot/feature_hist/feature_hist_baseline_strong.jpg')

...
def main():
    ...
    config = 'configs/zzp/baseline/baseline_prelu/baseline_prelu_14_b32x8/baseline_prelu_14_b32x8_step1.py'
    ckpt = 'work_dir/baseline/baseline_prelu/baseline_prelu_14_b32x8/baseline_prelu_14_b32x8_step1/epoch_75.pth'
    ...
    for i in range(16):
        ...
        axs[i].set_title(f'conv_in_{i}')
        axs[i].hist(blconv_in[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i].grid()
        axs[i+16].set_title(f'conv_out_{i}')
        axs[i+16].hist(blconv_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+16].grid()
        axs[i+32].set_title(f'prelu_out_{i}')
        axs[i+32].hist(prelu_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+32].grid()
        axs[i+48].set_title(f'sum_out_{i}')
        axs[i+48].hist(sum_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+48].grid()
        axs[i+64].set_title(f'bn_out_{i}')
        axs[i+64].hist(bn_out[i].flatten().numpy(), bins=100, alpha=0.7)
        axs[i+64].grid()
    fig.savefig('./work_dir/plot/feature_hist/feature_hist_baseline_prelu_14.jpg')
